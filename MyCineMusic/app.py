...
from queries import QUERY_BY_KEY, QUERY_SPACE

st.set_page_config(
    page_title="MyCineMusic Distributed DB", page_icon="🎬", layout="wide"
)

st.title("🎬 MyCineMusic Distributed Database Query Interface")
st.caption(
    "Streamlit client for the Docker/PostgreSQL distributed schema. Connect to the coordinator node and query reconstructed global views."
)
    ...
    for idx, spec in enumerate(query.params):
        ...
        with target:
            ...
            if ptype == "select":
                ...
                index = (
                    spec["options"].index(default) if default in spec["options"] else 0
                )
...
if run_button:
    try:
        ...
        st.caption(
            "Check that Docker is running, the coordinator is healthy, and the schema was initialized."
        )
